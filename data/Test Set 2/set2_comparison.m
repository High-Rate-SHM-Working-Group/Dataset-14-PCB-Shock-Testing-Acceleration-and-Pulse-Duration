...
%% Extract Data
DataFolder1 = 'C:\Users\AFRL_USER\Documents\2025 - Ryan Yount\Test Set 2\m018x1+T_p6KG\'; % choose unit to read
DataFolder2 = 'C:\Users\AFRL_USER\Documents\2025 - Ryan Yount\Test Set 2\m018x1+T3_p5-5KG\';

MainFolder = cd;
cd(DataFolder1) % step into folder
...
cd(MainFolder)
cd(DataFolder2) % step into folder
...
for k = 1:length(PFfiles)
    ...
    % Save results
    D(k).Filename = PFfilename;
    D(k).Time = time;
    D(k).Accel = Accel;
    D(k).Board = Board;
    D(k).Res = resistor;
    D(k).AccelFiltered = AccelFiltered;
    D(k).Peak = peak;
    D(k).Pulsewidth = wF;
    D(k).Velocity = vel;
    D(k).Freq = freq;
    D(k).AccelFFT = Accelfft;
    D(k).BoardFFT = Boardfft;
end
cd(MainFolder)
...
% hold on;
% yyaxis left
% plot(D(length(PFfiles)).Time, D(length(PFfiles)).Board, LineWidth=1.5)
% ylabel('Single Joint Failure (g_n)')
% ylim(base_ylim)
% yyaxis right
...
hold on;
for i = 1:length(PFfiles)
    plot(D(i).Freq, abs(D(i).BoardFFT), LineWidth=1.5)
end
ylabel('Single Joint Failure |g_n|')
ax2 = nexttile;
hold on;
